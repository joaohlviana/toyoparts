...
import { 
  ShoppingBag, Search, RefreshCw, Loader2, Calendar, FileJson 
} from 'lucide-react';
...
// Basic interface, as stored data might differ slightly or be a subset
...
export function StoredOrdersPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const url = new URL(`https://${projectId}.supabase.co/functions/v1/make-server-1d6e33e0/magento-sync/orders/stored`);
      ...
      if (!res.ok) throw new Error('Falha ao buscar pedidos armazenados');
    ...
    <div className="max-w-[1280px] mx-auto px-4 lg:px-6 pt-6 pb-12 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <ShoppingBag className="w-6 h-6" /> Pedidos (Banco de Dados)
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Pedidos importados e salvos no Supabase KV ({total} registros)
          </p>
        </div>
      ...
      <Sheet open={!!selectedOrder} onOpenChange={(open) => !open && setSelectedOrder(null)}>
        <SheetContent className="w-[400px] sm:w-[540px] overflow-y-auto">
          <SheetHeader>
            <SheetTitle>Dados Brutos do Pedido (KV)</SheetTitle>
            <SheetDescription>
