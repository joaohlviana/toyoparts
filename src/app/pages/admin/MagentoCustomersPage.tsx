...
import { fetchMagentoCustomers } from '../../lib/magento/magento-api';
import type { MagentoCustomer } from '../../lib/magento/magento-api';
...
export function MagentoCustomersPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await fetchMagentoCustomers(page, limit, search);
  ...
  const handleDebug = async () => {
    try {
      const res = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-1d6e33e0/access-links/debug`, {
        headers: {
          'Authorization': `Bearer ${publicAnonKey}`,
          'apikey': publicAnonKey
        }
    ...
    <div className="max-w-[1280px] mx-auto px-4 lg:px-6 pt-6 pb-12 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <Users className="w-6 h-6" /> Clientes Magento
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Visualização de clientes importados do Magento ({total} registros)
          </p>
        </div>
      ...
      <Dialog open={!!debugData} onOpenChange={(open) => !open && setDebugData(null)}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Debug Access Links (KV Store)</DialogTitle>
          </DialogHeader>
      ...
      <Sheet open={!!selectedCustomer} onOpenChange={(open) => !open && setSelectedCustomer(null)}>
        <SheetContent className="w-[400px] sm:w-[540px] overflow-y-auto">
          <SheetHeader>
            <SheetTitle>Dados Brutos do Cliente</SheetTitle>
            <SheetDescription>
